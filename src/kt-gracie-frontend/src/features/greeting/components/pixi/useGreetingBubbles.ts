...
export function useGreetingBubbles(count = 28) {
	...
	useEffect(() => {
		setBubbles((prev) =>
			prev.map((b) => ({
				...b,
				x: Math.min(b.x, viewport.width),
				y: Math.min(b.y, viewport.height),
			})),
		);
	}, [viewport]);
			...
			setBubbles((prev) =>
				prev.map((bubble) => {
					...
					if (nextY + bubble.radius < 0) {
						return { ...createBubble(bubble.id, viewport), y: viewport.height + bubble.radius };
					}
	...
	return useMemo(() => ({ bubbles, viewport }), [bubbles, viewport]);
}
